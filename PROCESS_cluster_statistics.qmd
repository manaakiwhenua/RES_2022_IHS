...
Note: Experimented unsuccessfully with z-score normalisation. Adding scaled x and y coordinates from cell centers also didn't improve the clustering stats
...
```{r}
cont_vars_df <- as.data.frame(covs, xy = TRUE) %>% 
...
invisible(purrr::map2(list(cm_03, cm_04, cm_05, cm_06, cm_07, cm_08, cm_09,
            ...
            seq(3,15,1),
            function(obj, i) {
              saveRDS(obj, file.path('models',
                                     paste0('cmeans_', i, '.rds')))
              }
            ))
```
...
Suggests minimal gains after n = 6 in this case, although S is still low at higher numbers, and it suggests that n = 3 and n = 4 aren't separating the dataset optimally. Plotting out the clusters spatially should be revealing. 

Note that *fuzzy c-means* isn't an explicitly spatially aware clustering algorithm, and can't simply be made so by e.g. adding cell center coordinates as clustering parameters. Other R packages like `geocmeans` may do a much better job with spatial datasets.
